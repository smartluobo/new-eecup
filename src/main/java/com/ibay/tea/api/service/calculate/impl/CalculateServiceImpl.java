package com.ibay.tea.api.service.calculate.impl;
...
public class CalculateServiceImpl implements CalculateService {
        ...
        if (cartItemIds != null && cartItemIds.trim().length() > 0) {
            ...
            if (tbActivity != null){
                if (ApiConstant.ACTIVITY_TYPE_1_1 == tbActivity.getActivityType()){
                    ...
                    LOGGER.info("buyOneGiveOneReduceAmount specialReduceAmount : {}",specialReduceAmount);
                }else if (ApiConstant.ACTIVITY_TYPE_TWO_HALF == tbActivity.getActivityType()){
                    specialReduceAmount = twoHalfReduceAmount(goodsList);
                    LOGGER.info("twoHalfReduceAmount specialReduceAmount : {}",specialReduceAmount);
                }
            }
            if (realGoodsCount >= 6){
                ...
                LOGGER.info("groupReduceAmount -----groupGiveName:{},groupGiveAmount : {}",groupGiveName,groupGiveAmount);
            }
            //如果总价大于100
            if (orderTotalPrice >= 100){
                ...
                LOGGER.info("fullReduceCalculate -----fullReduceName:{},fullReduceAmount : {}",fullReduceName,fullReduceAmount);
            }
            ...
            if (tbUserCoupons != null) {
                ...
                LOGGER.info("fullReduceCalculate -----couponsName:{},couponsReduceAmount : {}",couponsName,couponsReduceAmount);
            }
            ...
            if ( specialReduceAmount > 0){
                ...
                if (ApiConstant.ACTIVITY_TYPE_1_1 == tbActivity.getActivityType()){
                    ...
                    LOGGER.info("use special activity  ");
                }
            }
            //判断用户是否是vip公司用户
    ...
    private Map<String,Object> fullReduceCalculate(double orderTotalPrice) {
        ...
        for (TbCoupons tbCoupons : tbCouponsList) {
            LOGGER.info("goods full reduce calculate tbCoupons :{}",tbCoupons);
    ...
    private double twoHalfReduceAmount(List<TbItem> goodsList) {
        if (!CollectionUtils.isEmpty(goodsList)){
            ...
            if (goodsCount > 1){
                ...
                LOGGER.info("halfCount : {}",halfCount);
